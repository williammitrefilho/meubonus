...
class Session{
  ...
  public static function criar($usuario, $senha, $permissao){
    ...
    $query = "SELECT id AS uid FROM mb_usuarios WHERE usuario = '".$usuario."' AND senha = '".md5($senha)."' AND permissao >= '".$permissao."'";
    ...
    setcookie('id_sessao', $sid, 0, "/meubonus");    
    return true;
  }
  
  public static function encerrar(){
    
    setcookie('id_sessao', $sid, time() - 1, "/meubonus");
    return true;
  }
  ...
  public static function verificar(){
    ...
    $query = "SELECT u.id AS uid, u.nome, u.permissao FROM mb_sessoes s INNER JOIN mb_usuarios u ON s.usuario = u.id WHERE s.id = '".$sid."'";
    $stmt= $db->query($query);
    if(!$stmt){
